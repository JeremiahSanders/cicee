...
public static class EnvRequireHandling
{
  ...
  {
    return TryValidateRequest(dependencies, envRequireRequest).Bind(
      context => ProjectEnvironmentHelpers.ValidateEnvironment(
      ...
      context =>
      {
        dependencies.StandardOutWriteLine(obj: "Environment validation succeeded.");
        dependencies.StandardOutWriteLine($"  Metadata file: {context.FilePath}");
      }
    ).TapFailure(
      exception =>
      {
        dependencies.StandardErrorWriteLine(obj: "Environment validation failed.");
        dependencies.StandardErrorWriteLine($"  Reason: {exception.Message}");
      }
    ).Map(context => new EnvRequireResult(context.FilePath, context.ProjectMetadata)).AsTask();
  }
    ...
    EnvRequireRequest envRequireRequest)
  {
    ...
    {
      return ProjectMetadataLoader.TryFindProjectMetadata(
