...
public static class MetaCiEnvVarRemoveHandling
{
  public static async Task<Result<ProjectEnvironmentVariable[]>> MetaCiEnvVarRemoveRequest(
    CommandDependencies dependencies, string projectMetadataPath, string name, bool isDryRun = false)
  {
    ...
    return await ProjectMetadataLoader
      .TryLoadFromFile(dependencies.EnsureFileExists, dependencies.TryLoadFileString, projectMetadataPath).Map(
        metadata => metadata.CiEnvironment.Variables.Fold(
          ...
          (enumerables, variable) => variable.Name.ToUpperInvariant().Equals(comparisonName)
        ...
        enumerables => new
        {
          Matches = enumerables.Matches.ToArray(), NonMatches = enumerables.NonMatches.ToArray()
        }
      ).Filter(
        ...
        async variables =>
        {
          return isDryRun
            ...
            : (await ProjectMetadataManipulation.UpdateVariablesInMetadata(
              dependencies,
              projectMetadataPath,
              variables.NonMatches
            )).Map(_ => variables.Matches);
        }
      );
  }
}
