...
public static class MetaCiEnvVarAddHandling
{
  ...
  {
    return !metadata.CiEnvironment.Variables
      .Select(ciVariable => ciVariable.Name.ToUpperInvariant())
  ...
  )
  {
    return await ProjectMetadataLoader.TryLoadFromFile(
      ...
      .Filter(metadata => DoesNotContainVariable(metadata, variable),
        _ => new ExecutionException($"Variable \"{variable.Name}\" already exists.", exitCode: 1)
      )
      .BindAsync(async metadata =>
      {
        var newVariables = new[] { variable };
        var updatedVariables = metadata.CiEnvironment.Variables.Append(newVariables).ToArray();
        return isDryRun
