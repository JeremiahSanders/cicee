...
public static class RepositoryEntrypoint
{
  public static async Task<Result<RepositoryResult>> TryHandleAsync(CommandDependencies dependencies,
    string projectRoot, string? image, bool force,
    bool initCiLib, LibraryShellTemplate? shell)
  {
    return await (await InitEntrypoint.TryHandleAsync(dependencies, projectRoot, image, force))
      .BindAsync(async initResult =>
        await (await TemplateInitEntrypoint.TryHandleAsync(dependencies, projectRoot, force))
          .BindAsync(
