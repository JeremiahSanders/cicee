  ...
  public static IEnumerable<object[]> GenerateTestCases()
  {
    ...
    ProjectMetadata baseArrangedMetadata = new()
    {
      ...
      CiEnvironment = new ProjectContinuousIntegrationEnvironmentDefinition
      {
        Variables = new[]
        {
          GenerateRandomEnvironmentVariable()
        }
      }
    };
    ...
    CommandDependencies dependencies = DependencyHelper.CreateMockDependencies() with
    {
      ...
      path => path == arrangedProjectRoot
        ...
        : new Result<string>(new Exception(message: "Directory not found")),
    ...
    EnvRequireRequest happyPathFile = new(ProjectRoot: null, combinePath(arrangedProjectRoot, arrangedProjectMetadata));
    ...
    Result<EnvRequireResult> happyPathResult = new(
      new EnvRequireResult(happyPathFile.ProjectMetadataFile!, baseArrangedMetadata)
    );
